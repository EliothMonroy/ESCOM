...
class ManejadorWav:
	def __init__(self,tempo,instrumento,sincronizacion,arreglo_notas):
		...
		if os.path.exists("salida.wav"):
			...
			self.salida=self.ajustarWav(self.salida)
	def crearWav(self):
	...
	def tocarMismoTiempo(self,a,b):
		...
		diferencia=len(muestras1)-len(muestras2)
		#Ajuste por si el tamaño de los arreglos no es exactamente el mismo
		#Aunque los archivos tengan una misma duración y fm pueden no tener el mismo número
		#de muestras 
		muestras=None
		if diferencia>0:
			for i in range(diferencia):
				muestras2.append(0)
				muestras3=[None] * len(muestras1)
		else:
			for i in range(-diferencia):
				muestras1.append(0)
	...
	def insertarSilencio(self,tiempo):
		return self.ajustarWav(AudioSegment.empty())+AudioSegment.silent(duration=tiempo,frame_rate=44100)
	def ajustarWav(self,a):
		a=a.set_channels(2)
