...
function obtenerFuncion(){
	return parseInt(document.getElementById("funcion").value);
}

function obtenerTau(){
	tau=parseInt(document.getElementById("tau").value);
	historico=new Array(tau);
	for(var i=0;i<tau;i++){
		historico[i]=Create2DArray(tam);
	}
}
...
function iniciar(){
	...
	historico[contador]=copiar(array2);
	contador++;
}

function dibujarMatrizAuxiliar(i,j,valor){
	if (valor==1) {
		ctx2.fillStyle=color_vivo;
		numero_vivos++;
	}else{
		ctx2.fillStyle=color_muerto;
		numero_muertos++;
	}
	ctx2.fillRect(0+(j*(escala)),0+(i*(escala)),escala,escala);
	ctx2.stroke();
}

function generarMatrizAuxiliar(){
	var funcion=obtenerFuncion();
	var valor;
	for(var i=0;i<tam;i++){
		for(var j=0;j<tam;j++){
			if(funcion==0){//Máximo
			...
			}else if(funcion==2){//Paridad
				var aux=0;
				for(var k=0;k<tau;k++){
					//console.log(historico[k][i][j]);
					//console.log("k: "+k);
					aux+=historico[k][i][j];
				}
				//console.log("aux: "+aux);
			...
			dibujarMatrizAuxiliar(i,j,valor);
			array[i][j]=valor;
		}
	}
}
...
function ejecutar(){
	...
	if(contador==tau){
		...
		numero_vivos=0;
		numero_muertos=0;
		console.log("Voy a generar la matriz auxiliar");
		generarMatrizAuxiliar();
		array2=copiar(array);
		console.log("Vivos en auxiliar "+numero_vivos);
	}
	for(var i=0; i<tam;i++){
		for(var j=0;j<tam;j++){
			evaluar(i,j);
